...
async function loadSections() {
    ...
    floors.forEach(floor => {
        ...
        div.className = 'bg-white p-6 md:p-8 rounded-[2rem] shadow-sm mb-6';
        ...
        let issuesHtml = floorIssues.map(i => {
            ...
            return `
                <div class="flex justify-between items-start italic mb-8 last:mb-0">
                    <div class="flex-1 italic">
                        <p class="text-[8px] font-black text-slate-400 uppercase mb-1">${i.locations?.name || '--'}</p>
                        <p class="text-sm font-bold ${isDone ? 'text-green-600' : 'text-slate-800'} mb-1 font-bold">${i.title}</p>
                        <p class="text-[8px] text-slate-400 font-bold uppercase italic font-bold">Nahlásil: ${fLog ? fmtD(fLog.event_date) : '--'} ${i.reported_by || '--'} • Zodpovedný: ${i.responsible_person || '--'}</p>
                    </div>
                    <div class="flex items-center space-x-3 ml-4 leading-tight leading-tight leading-tight"><div class="flex items-center leading-none">${photos}</div><button onclick="window.prepStat('${i.id}')" class="bg-white px-3 py-1.5 rounded-lg border border-slate-100 text-[9px] font-black uppercase text-blue-600 underline italic">Upraviť</button></div>
                </div>`;
        }).join('');
        div.innerHTML = `<div class="flex justify-between items-center border-b pb-4 mb-4"><h3 class="font-black text-xl italic uppercase text-slate-900">${floor}</h3><button onclick="window.prepAdd('${floor}')" class="bg-slate-900 text-white px-5 py-2 rounded-xl text-[9px] font-black uppercase tracking-widest leading-none">+ Pridať</button></div><div>${issuesHtml || '<p class="text-center py-6 text-[10px] text-slate-200 font-bold uppercase font-bold">Budova OK</p>'}</div>`;
...
async function loadArchive() {
    ...
    container.innerHTML = arch?.length ? arch.map(i => `
        <div class="bg-white p-4 rounded-2xl shadow-sm flex justify-between items-center italic mb-2">
            <div><p class="text-[8px] font-black text-slate-400 uppercase">${i.locations?.name || '--'}</p><p class="text-[13px] font-bold text-slate-600">${i.title}</p></div>
            <button onclick="restoreIssue('${i.id}')" class="text-[10px] font-black uppercase text-blue-600 underline italic">Vrátiť</button>
        </div>`).join('') : '<div class="py-20 text-center text-slate-200 font-black uppercase text-[10px] font-bold">Archív je prázdny</div>';
}

async function loadReports() {
    const todayStr = new Date().toLocaleDateString(); document.getElementById('rep-date-screen').innerText = todayStr;
    ...
    list.innerHTML = isss.map(i => {
        ...
        return `<tr class="rep-row italic"><td class="py-5 px-2 align-top border-r border-slate-50 italic"><span class="block font-black text-slate-400 uppercase text-[7px]">${i.locations.floor}</span><span class="text-[10px] font-bold">${i.locations.name}</span><p class="text-[7px] font-bold text-slate-400 uppercase mt-2">Zodpovedá: ${i.responsible_person || '--'}</p></td><td class="py-5 px-3 align-top italic"><p class="font-bold text-slate-900 mb-3 font-bold">${i.title}</p><div class="space-y-4">${logs.map(u => `<div class="flex justify-between items-start space-x-2 pb-1 italic"><div class="flex-1"><div class="flex items-center space-x-2 mb-1"><span class="font-black text-[7px] text-slate-400 uppercase font-bold">${fmtD(u.event_date)}</span><span class="text-[6px] font-black px-1 border rounded uppercase ${u.status_to === 'Opravené' || u.status_to === 'Vybavené' ? 'text-green-600' : 'text-slate-400'}">${u.status_to}</span></div><p class="text-[9px] text-slate-700 leading-snug">${u.note || '--'}</p></div>${u.photo_url ? `<img src="${u.photo_url}?width=150&quality=20" class="report-thumb" onclick="window.open('${u.photo_url}')">` : ''}</div>`).join('')}</div></td><td class="py-5 px-1 align-top text-center"><span class="text-[7px] font-black px-1.5 py-0.5 rounded uppercase ${i.status === 'Opravené' || i.status === 'Vybavené' ? 'text-green-600 bg-green-50' : 'text-red-500 bg-red-50'}">${i.status}</span></td></tr>`;
    }).join('');
}
...
window.prepStat = (id) => {
    ...
    document.getElementById('m-history-list').innerHTML = logs.map(u => `
        <div class="p-3 bg-slate-50 rounded-xl border border-slate-100 text-[10px] mb-4 italic leading-tight">
            <div class="flex justify-between items-start mb-1 leading-tight"><span class="font-black block text-slate-800 uppercase tracking-tighter">${fmtD(u.event_date)} • ${u.status_to}</span><div class="flex space-x-2 leading-tight leading-tight"><button type="button" onclick="window.editHEntry('${u.id}')" class="text-blue-500 italic leading-none italic leading-tight"><i class="fa-solid fa-pencil italic"></i></button><button type="button" onclick="window.delHEntry('${u.id}')" class="text-red-300 italic leading-none italic leading-tight"><i class="fa-solid fa-trash-can italic"></i></button></div></div>
            <div class="grid grid-cols-2 gap-2 text-[8px] font-bold uppercase text-slate-500 italic mb-2 leading-tight">
