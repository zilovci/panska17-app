...
async function loadSections() {
    ...
    container.innerHTML = '<div class="py-20 text-center animate-pulse text-slate-300 uppercase italic font-bold">Sync...</div>';
    ...
    container.innerHTML = ''; const floors = [...new Set(allLocs.map(l => l.floor))];
    floors.forEach(floor => {
        const floorLocs = allLocs.filter(l => l.floor === floor); const floorIssues = allIssues.filter(i => floorLocs.some(l => l.id === i.location_id));
        const div = document.createElement('div'); div.className = 'bg-white p-6 md:p-8 rounded-[2rem] border-none shadow-sm space-y-4 italic leading-tight mb-6';
        let issuesHtml = floorIssues.map(i => {
            ...
            const isDone = i.status === 'Opravené' || i.status === 'Vybavené';
            const fLog = logs.length > 0 ? logs[logs.length-1] : null;
            return `<div class="p-0 flex justify-between items-start italic mb-6 last:mb-0"><div class="flex-1 italic"><p class="text-[8px] font-black text-slate-400 uppercase leading-none mb-1">${i.locations?.name || '--'}</p><p class="text-sm font-bold ${isDone ? 'text-green-600' : 'text-slate-800'} italic leading-tight mb-1">${i.title}</p><p class="text-[8px] text-slate-400 font-bold uppercase italic italic">Nahlásil: ${fLog ? fmtD(fLog.event_date) : '--'} ${i.reported_by || '--'} • Zodpovedný: ${i.responsible_person || '--'}</p></div><div class="flex items-center space-x-3 ml-4 leading-tight"><div class="flex items-center leading-none">${photos}</div><button onclick="window.prepStat('${i.id}')" class="bg-white px-3 py-1.5 rounded-lg border border-slate-100 text-[9px] font-black uppercase text-blue-600 underline italic leading-tight">Upraviť</button></div></div>`;
        }).join('');
        div.innerHTML = `<div class="flex justify-between items-center border-b pb-4 mb-4 italic"><h3 class="font-black text-xl italic uppercase text-slate-900">${floor}</h3><button onclick="window.prepAdd('${floor}')" class="bg-slate-900 text-white px-5 py-2 rounded-xl text-[9px] font-black uppercase tracking-widest leading-none">+ Pridať</button></div><div class="space-y-4 italic leading-tight leading-tight">${issuesHtml || '<p class="text-center py-6 text-[10px] text-slate-200 font-bold uppercase tracking-widest">Budova OK</p>'}</div>`;
...
async function loadReports() {
    ...
    const list = document.getElementById('rep-list'); const { data: isss } = await sb.from('issues').select('*, locations(*)').eq('archived', false);
    ...
    list.innerHTML = isss.map(i => {
        ...
        return `<tr class="rep-row italic leading-snug"><td class="py-5 px-2 align-top border-r border-slate-50 italic leading-tight"><span class="block font-black text-slate-400 uppercase text-[7px] italic">${i.locations.floor}</span><span class="text-[10px] font-bold italic">${i.locations.name}</span><p class="text-[7px] font-bold text-slate-400 uppercase mt-2 italic">Zodpovedá: ${i.responsible_person || '--'}</p></td><td class="py-5 px-3 align-top italic leading-snug leading-tight"><p class="font-bold text-slate-900 italic mb-3 leading-tight italic leading-tight">${i.title}</p><div class="space-y-4 italic leading-tight">${logs.map(u => `<div class="flex justify-between items-start space-x-2 pb-1 italic leading-tight"><div class="flex-1 italic"><div class="flex items-center space-x-2 mb-1 italic leading-tight leading-tight"><span class="font-black text-[7px] text-slate-400 uppercase italic leading-tight italic leading-tight">${fmtD(u.event_date)}</span><span class="text-[6px] font-black px-1 border rounded uppercase italic leading-tight italic leading-tight ${u.status_to === 'Opravené' || u.status_to === 'Vybavené' ? 'text-green-600' : 'text-slate-400'}">${u.status_to}</span></div><p class="text-[9px] text-slate-700 italic leading-snug leading-tight">${u.note || '--'}</p></div>${u.photo_url ? `<img src="${u.photo_url}?width=150&quality=20" class="report-thumb cursor-pointer italic" onclick="window.open('${u.photo_url}')">` : ''}</div>`).join('')}</div></td><td class="py-5 px-1 align-top text-center italic leading-tight leading-tight italic leading-tight"><span class="text-[7px] font-black px-1.5 py-0.5 rounded uppercase italic leading-tight ${i.status === 'Opravené' || i.status === 'Vybavené' ? 'text-green-600 bg-green-50' : 'text-red-500 bg-red-50'}">${i.status}</span></td></tr>`;
    }).join('');
}

async function loadArchive() {
    const container = document.getElementById('archive-container');
    const { data: arch } = await sb.from('issues').select('*, locations(*)').eq('archived', true).order('updated_at', { ascending: false });
    container.innerHTML = arch?.length ? arch.map(i => `<div class="bg-white p-5 rounded-2xl border-none shadow-sm flex justify-between items-center italic"><div><p class="text-[8px] font-black text-slate-400 uppercase tracking-widest leading-none">${i.locations?.name || '--'}</p><p class="text-[13px] font-bold text-slate-600 italic leading-tight">${i.title}</p></div><button onclick="restoreIssue('${i.id}')" class="text-[10px] font-black uppercase text-blue-600 underline leading-none italic italic">Vrátiť</button></div>`).join('') : '<div class="py-20 text-center text-slate-200 font-black uppercase text-[10px] italic tracking-widest">Prázdne</div>';
}

window.restoreIssue = async (id) => {
    if(confirm("Vrátiť z archívu?")) {
...
window.prepStat = (id) => {
    ...
    document.getElementById('m-history-list').innerHTML = logs.map(u => `
        <div class="p-3 bg-slate-50 rounded-xl border-none text-[10px] mb-2 italic leading-tight">
            <div class="flex justify-between items-start mb-1 italic leading-tight"><span class="font-black block text-slate-800 uppercase tracking-tighter italic leading-tight">${fmtD(u.event_date)} • ${u.status_to}</span><div class="flex space-x-2 italic leading-tight leading-tight leading-tight"><button type="button" onclick="window.editHEntry('${u.id}')" class="text-blue-500 italic leading-none italic leading-tight"><i class="fa-solid fa-pencil italic"></i></button><button type="button" onclick="window.delHEntry('${u.id}')" class="text-red-300 italic leading-none italic leading-tight"><i class="fa-solid fa-trash-can italic"></i></button></div></div>
            <div class="grid grid-cols-2 gap-2 text-[8px] font-bold uppercase text-slate-500 italic mb-2 italic"><p>Nahlásil: ${u.attendance || '--'}</p><p>Zodpovedný: ${item.responsible_person || '--'}</p></div>
            <p class="text-slate-500 leading-snug italic italic">${u.note || '--'}</p>
