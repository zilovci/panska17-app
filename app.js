...
document.getElementById('f-login').onsubmit = async (e) => {
    e.preventDefault();
    const btn = document.getElementById('btn-login'); btn.innerText = "Overujem...";
...
async function loadSections() {
    ...
    floors.forEach(floor => {
        ...
        let issuesHtml = floorIssues.map(i => {
            ...
            return `
                <div class="flex justify-between items-start mb-8 last:mb-0">
                    <div class="flex-1">
...
async function loadReports() {
    ...
    document.getElementById('rep-date-screen').innerText = today;
    ...
    list.innerHTML = isss.map(i => {
        ...
        return `<tr class="rep-row"><td class="py-5 px-2 align-top border-r border-slate-50 italic"><span class="block font-black text-slate-400 uppercase text-[7px]">${i.locations.floor}</span><span class="text-[10px] font-bold">${i.locations.name}</span><p class="text-[7px] font-bold text-slate-400 uppercase mt-2">Zodpovedá: ${i.responsible_person || '--'}</p></td><td class="py-5 px-3 align-top italic"><p class="font-bold text-slate-900 mb-3">${i.title}</p><div class="space-y-4">${logs.map(u => `<div class="flex justify-between items-start space-x-2 pb-1"><div class="flex-1"><div class="flex items-center space-x-2 mb-1"><span class="font-black text-[7px] text-slate-400 uppercase">${fmtD(u.event_date)}</span><span class="text-[6px] font-black px-1 border rounded uppercase ${u.status_to === 'Opravené' || u.status_to === 'Vybavené' ? 'text-green-600' : 'text-slate-400'}">${u.status_to}</span></div><p class="text-[9px] text-slate-700 leading-snug">${u.note || '--'}</p></div>${u.photo_url ? `<img src="${u.photo_url}?width=150&quality=20" class="report-thumb" onclick="window.open('${u.photo_url}')">` : ''}</div>`).join('')}</div></td><td class="py-5 px-1 align-top text-center"><span class="text-[7px] font-black px-1.5 py-0.5 rounded uppercase ${i.status === 'Opravené' || i.status === 'Vybavené' ? 'text-green-600 bg-green-50' : 'text-red-500 bg-red-50'}">${i.status}</span></td></tr>`;
    }).join('');
}
...
window.prepStat = (id) => {
    ...
    document.getElementById('m-history-list').innerHTML = logs.map(u => `
        <div class="p-3 bg-slate-50 rounded-xl border border-slate-100 text-[10px] mb-4 italic">
            <div class="flex justify-between items-start mb-2">
                <span class="font-black block text-slate-800 uppercase">${fmtD(u.event_date)} • ${u.status_to}</span>
                <div class="flex space-x-2"><button type="button" onclick="editHEntry('${u.id}')" class="text-blue-500"><i class="fa-solid fa-pencil"></i></button><button type="button" onclick="delHEntry('${u.id}')" class="text-red-300"><i class="fa-solid fa-trash-can"></i></button></div>
            </div>
            <div class="grid grid-cols-2 gap-2 text-[8px] font-bold uppercase text-slate-500 mb-2">
                <p>Nahlásil: ${u.attendance || '--'}</p><p>Zodpovedný: ${item.responsible_person || '--'}</p>
            </div>
            <p class="text-slate-500 leading-snug">${u.note || '--'}</p>
            ${u.photo_url ? `<img src="${u.photo_url}?width=300&quality=30" class="app-thumb mt-2" onclick="window.open('${u.photo_url}')">` : ''}
        </div>`).join('');
...
window.delHEntry = async (id) => { if(confirm("Zmazať bod?")) { await sb.from('issue_updates').delete().eq('id', id); window.prepStat(document.getElementById('f-stat-id').value); } };
