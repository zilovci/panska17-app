...
async function loadAdmin() {
  ...
  var roleLabels = { admin: 'Admin', ekonom: 'Ekonóm', spravca: 'Správca', zastupca: 'Zástupca', pracovnik: 'Pracovník', pozorovatel: 'Pozorovateľ' };
  ...
  document.getElementById('admin-user-list').innerHTML = users.length === 0
    ...
    : users.map(u => {
      ...
      var isAdminOrSpravca = u.role === 'admin' || u.role === 'ekonom' || u.role === 'spravca' || u.role === 'zastupca';
        ...
        <div class="flex items-center justify-between">
          ...
          <div class="flex items-center space-x-3">
            <select onchange="window.changeUserRole('${u.id}', this.value)" class="text-[10px] font-bold border border-slate-200 rounded-lg px-2 py-1 ${u.user_id === currentUserId ? 'opacity-50' : ''}" ${u.user_id === currentUserId ? 'disabled' : ''}>
              ${['admin','ekonom','spravca','zastupca','pracovnik','pozorovatel'].map(r => `<option value="${r}" ${u.role === r ? 'selected' : ''}>${roleLabels[r]}</option>`).join('')}
            </select>
...
window.generateInvoice = async function(existingInvoice) {
  ...
  if (tenant.ico) { doc.text(stripDia('IČO: ' + tenant.ico), colR, ty); ty += 4; }
  if (tenant.dic) { doc.text(stripDia('DIČ: ' + tenant.dic), colR, ty); ty += 4; }
  if (tenant.ic_dph) { doc.text(stripDia('IČ DPH: ' + tenant.ic_dph), colR, ty); ty += 4; }
  ...
  doc.autoTable({
    ...
    columnStyles: {
      ...
      2: { cellWidth: 40, halign: 'right', cellPadding: { top: 2, bottom: 2, left: 2, right: 5 } }
    }
  });
  ...
  if (hasWater || hasElec || hasHeat || hasEps) {
    ...
    function detailSection(title, rows) {
      ...
      doc.autoTable({
        ...
        columnStyles: { 0: { cellWidth: 55 }, 1: { cellWidth: 35, halign: 'right' }, 2: { cellWidth: 40, halign: 'right' } }
      });
...
async function init() {
  ...
  if (session) {
    ...
    // Admin/spravca/zastupca sees all zones
